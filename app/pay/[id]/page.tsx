...
import { PayMonButton } from "@/components/pay-mon-button"
        ...
        <div className="flex min-h-screen flex-col">
          ...
          <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-6 py-16">
            <div className="rounded-2xl border border-neutral-700 bg-neutral-900/80 p-8 shadow-xl">
              ...
              <p className="mt-2 text-center text-neutral-400">Monto a pagar (MON nativo)</p>
              <p className="mt-4 text-center text-4xl font-bold text-white">
                {amount.toLocaleString("es", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} MON
              </p>
              <p className="mt-2 text-center text-sm text-neutral-500">A {shortenAddress(id)}</p>
              <div className="mt-8">
                <PayMonButton vendorAddress={id} amount={amount} />
              </div>
            </div>
          </main>
        </div>
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-6 py-16">
        <div className="rounded-2xl border border-neutral-700 bg-neutral-900/80 p-8 shadow-xl">
          <h1 className="text-center text-2xl font-bold text-white">Pago en USDC</h1>
                    ...
                    <p className="mt-2 text-center text-neutral-400">Monto a pagar en Monad Testnet (USDC)</p>
          <p className="mt-4 text-center text-4xl font-bold text-white">
            {paymentLink.amount.toLocaleString("es", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDC
          </p>
          <div className="mt-8">
            <PayButton
